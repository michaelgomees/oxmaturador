...
CREATE TABLE public.saas_usuarios (
  ...
  chips_limite INTEGER NOT NULL DEFAULT 1,
  status TEXT NOT NULL DEFAULT 'ativo' CHECK (status IN ('ativo', 'inativo')),
  created_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now(),
  updated_at TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT now()
);

-- Criar tabela de conexões/chips
CREATE TABLE public.saas_conexoes (
  id UUID NOT NULL DEFAULT gen_random_uuid() PRIMARY KEY,
  usuario_id UUID NOT NULL REFERENCES public.saas_usuarios(id) ON DELETE CASCADE,
  nome TEXT NOT NULL,
  config JSONB,
...
-- Usuários podem atualizar apenas seus próprios dados (exceto chips_limite)
...
INSERT INTO public.saas_usuarios (nome, email, senha_hash, chips_limite, status)
VALUES ('Admin', 'admin@admin.com', '$2b$10$example_hash_here', 10, 'ativo');